...
// Generate product HTML for Top10 blog posts
export function generateProductHtml(product: any, rank: number): string {
  ...
  const image = product.image_url || product.image || product.imageUrl || 'https://via.placeholder.com/300x200?text=Lenovo+Laptop';
  ...
  return `
    <div class="product-card w-full clear-both flex flex-col md:flex-row overflow-hidden rounded-lg shadow-lg border border-gray-200 my-8 hover:shadow-xl transition-shadow bg-white" data-asin="${escapeHtml(asin)}" data-rank="${rank}">
